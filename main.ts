...
type Score = 'A' | 'B' | 'C' | 'D' | 'F'; //정해져있는 특정값을 그룹으로 묶어서 커스텀 타입 지정가능
...
interface Student {
	name: string;
	age: number;
	readonly isFemale: boolean; //해당 property는 읽기전용으로 변경불가
	address?: string; //해당 poperty는 optional하게 적용
	[Grade: number]: Score; //property명을 산정할수 없을때 대괄호로 묶어서 타입지정 가능
}
